...
p3 <- facet_plot(p2, panel='Relative cornea size', data=sal.phy, geom=geom_segment, aes(x=1, xend=pglsres.cdmass, y=labels, yend=labels, color = Adult_Habitat), size=3, alpha = 0.7) +
...
# Make the phylogeny plot
p_blind <- ggtree(caudatatree_blind) %<+% sal.phy.blind + 
     geom_tiplab(size = 3, aes(label = labels), fontface = 3) + 
   xlim_tree(375) + 
   coord_cartesian(clip = 'off') 
plot(p_blind)
...
# Make the phylogeny plot
p_act <- ggtree(caudatatree) %<+% sal.phy + 
  geom_tiplab(size = 3, aes(label = labels), fontface = 3) + 
  xlim_tree(375) + 
  coord_cartesian(clip = 'off') 
